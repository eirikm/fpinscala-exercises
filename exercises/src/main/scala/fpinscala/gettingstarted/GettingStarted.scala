...
object MyModule { 
  ...
  def fib(n: Int): Int = {
    @annotation.tailrec
    def loop(prev: Int, current: Int, left: Int): Int = {
      left match {
        case 0 => prev
        case _ => loop(current, prev + current, left - 1)
      }
    }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    @annotation.tailrec
    def loop(prev: A, as: Array[A]): Boolean = {
      if (as.isEmpty) true
      else {
        if (gt(prev, as.head)) loop(as.head, as.tail)
        else false
      }
    }

    if (as.length < 2) true
    else loop(as(0), as.tail)
  }
